#!/usr/bin/env python3
"""
Extract GEO with Pricing v22.3 - SBD Name Reformatting
======================================================
...
OUTPUT_FILE = OUTPUT_DIR / "full_geo_extraction_v22.3.jsonl"
...
RELEASE_MODIFIERS = ['12 HR', '24 HR', '72 HR', 'SR', 'ER', 'XR', 'CR', 'LA', 'SA', 'XL']
...
def extract_brand(name, fallback_brand=None):
    ...
    clean_name = name[:last_open].strip()
...
def extract_container_size(name):
    """Extract container size from start (e.g., '0.1 ML', '250 ML')."""
    match = re.match(r'^(\d+(?:\.\d+)?)\s*(ML|L)\s+', name, re.IGNORECASE)
    if match:
        container = f"{match.group(1)} {match.group(2).upper()}"
        clean_name = name[match.end():].strip()
        return container, clean_name
    return None, name
...
def reformat_sbd_name(name, brand_name_from_bn=None):
    ...
    """
    # Extract brand (with fallback from BN relationship)
    ...
    # Extract container size if present at START
    ...
    if ' / ' in name_after_container:
        ...
        if parsed:
            result = f"{brand} [{parsed['ingredients']}] {parsed['doses']} {parsed['dose_form']}"
            if container:
                result += f" {container}"
            if extra_dose:
                result += f" {extra_dose}"
            ...
            if child_tty == 'SCDC':
                for rel2, target_id in forward.get(child_id, []):
                    ...
                    if target_tty == 'SCD' and target_rxcui in found_scds:
                        is_combo, ingredients = is_combo_scd(target_id, forward, reverse, entity_id_to_entity)
                        
                        if is_combo:
                            for mid, ment in found_mins.items():
                                min_ings = {i['rxcui'] for i in get_ingredients_from_min(ment['id'], forward, entity_id_to_entity)}
                                if min_ings == {i['rxcui'] for i in ingredients}:
                        ...
                        for rel3, df_id in reverse.get(target_id, []):
...
def main():
    parser = argparse.ArgumentParser(description='Extract GEO v22.3 - SBD Name Reformatting')
    parser.add_argument('--debug', action='store_true')
    args = parser.parse_args()
    
    print("=" * 80)
    print("EXTRACT GEO V22.3 - SBD Name Reformatting")
    ...
    print(f"\nV22.3: SBD names reformatted with brand first, NDA/ANDA stripped")
    print("=" * 80)
