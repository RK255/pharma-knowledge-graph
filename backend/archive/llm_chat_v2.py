...
- clinical_priority: PRIMARY (recommend first), SECONDARY (alternative if PRIMARY fails), CAUTION (limited use)
...
**USE WITH CAUTION** (clinical_priority: CAUTION):
• [Drug Name] - Weight: [X]/100
  - Reason: [weight_rationale]
  - Evidence: [weight_evidence]

EXAMPLE for "alternatives to simvastatin":
1. search_drugs("simvastatin") → returns drug_id: "7dbb03eb94c1cc69"
2. get_related_drugs("7dbb03eb94c1cc69") → returns weighted alternatives
3. Present pitavastatin (PRIMARY, 90/100) FIRST, then ezetimibe (SECONDARY, 80/100)
...
INDICATION-SPECIFIC WEIGHTING:
If user mentions a specific condition (e.g., "for statin intolerance"), pass it as the indication parameter to get_related_drugs for re-weighted recommendations.
