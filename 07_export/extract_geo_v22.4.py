...
def extract_container_size(name):
    """Extract container size from start (e.g., '0.1 ML', '250 ML', '200 ACTUAT')."""
    # Match: NUMBER UNIT at start, only if there's another dose pattern later
    match = re.match(r'^(\d+(?:\.\d+)?)\s*(ML|L|ACTUAT)\s+(.+)$', name, re.IGNORECASE)
    if match:
        rest = match.group(3)
