...
class ProvenanceManager:
    ...
    def __init__(self, base_dir: str = None):
        ...
        # Helper function to get ID from schema (handles both string ID and dict)
        def get_id(obj):
        ...
        self.HAS_PROVENANCE_REL_ID = get_id(self.schema.rel("has_provenance"))
        self.PROVENANCE_TYPE_ID = get_id(self.schema.type_id("Provenance"))
        
        # Property IDs
        self.prop_name = get_id(self.schema.prop("name"))
        self.prop_source = get_id(self.schema.prop("source"))
        self.prop_citation = get_id(self.schema.prop("citation"))
        self.prop_date_accessed = get_id(self.schema.prop("date_accessed"))
        self.prop_source_url = get_id(self.schema.prop("source_url"))
        self.prop_provenance_type = get_id(self.schema.prop("provenance_type"))
    ...
    def _is_provenance_entity(self, entity: dict) -> bool:
        ...
        return (self.PROVENANCE_TYPE_ID in types or 
                "Provenance" in types or
                entity.get('type') == "Provenance" or
                entity.get('type') == self.PROVENANCE_TYPE_ID)
    ...
    def get_coverage_stats(self, entities: List[dict], relations: List[dict]) -> Dict[str, Any]:
        ...
        for entity in entities:
            if self._is_provenance_entity(entity):
                ...
                # Extract name from values (GRC-20 format stores it in values)
                name = None
                for val in entity.get('values', []):
                    if val.get('property') == self.prop_name:
                        name = val.get('value')
                        break
                
                # Fallback to ID if name not found
                self.provenance_entities[name or entity['id']] = entity
...
def main():
    ...
    if args.fix and stats['coverage'] < 100:
        # Create pipeline_generated provenance (optional - comment out to skip)
