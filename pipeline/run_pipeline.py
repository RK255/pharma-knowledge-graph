...
def ensure_provenance_coverage(limit=None):
    ...
    pm = ProvenanceManager(str(BASE_DIR))
    
    # Load merged entities
    input_file = DATA_DIR / "grc20_merged.json"
    entities = pm.load_entities(str(input_file))
    ...
    if stats['coverage'] < 100:
        print(f"  Adding provenance to {stats['total'] - stats['with_provenance']} entities...")
        
        # Create pipeline_generated provenance if needed
        if 'pipeline_generated' not in pm.provenance_entities:
        ...
        # Apply to entities without provenance
        updated = pm.apply_type_provenance_defaults(entities)
        print(f"  Updated {updated} entities")
        
        # Save
        output_file = DATA_DIR / "grc20_with_relations.json"
        pm.save_entities(entities, str(output_file))
        
        # Verify
        new_stats = pm.get_coverage_stats(entities)
